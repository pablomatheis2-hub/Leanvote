...
async function getInProgressCount(boardOwnerId: string): Promise<number> {
  const supabase = await createClient();
...
export default async function DashboardChangelogPage() {
  ...
  const [profile, posts, inProgressCount] = await Promise.all([
    getProfile(user.id),
    getCompletedPosts(user.id),
    getInProgressCount(user.id),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        {profile?.board_slug && (
          <Link
            href={`/b/${profile.board_slug}/changelog`}
            target="_blank"
